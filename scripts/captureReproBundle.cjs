...
async function main() {
    ...
    await fs.writeFile(jsonPath, bundleJson, 'utf8');

    const gzipPath = path.join(outputDir, 'readme-repro-bundle.json.gz');
    await fs.writeFile(gzipPath, await gzip(Buffer.from(bundleJson, 'utf8')));
